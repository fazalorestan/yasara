...
class BitunixAdapterV1(BaseExchangeAdapterV1):
    exchange = SupportedExchange.BITUNIX
    ...
    def descriptor(self) -> ExchangeDescriptor:
        return ExchangeDescriptor(
            ...
            notes="Bitunix public market data scaffold added. Live trading remains disabled.",
        )

    async def ticker(self, symbol: str) -> UnifiedTicker:
        mapped = self.symbols.to_exchange_symbol(self.exchange, symbol)
        payload = await self.client.get_json("https://api.bitunix.com/api/spot/v1/market/ticker", {"symbol": mapped})
    ...
    async def order_book(self, symbol: str, limit: int = 20) -> UnifiedOrderBook:
        mapped = self.symbols.to_exchange_symbol(self.exchange, symbol)
        payload = await self.client.get_json("https://api.bitunix.com/api/spot/v1/market/depth", {"symbol": mapped, "limit": limit})
    ...
    async def place_order_dry_run(self, request: UnifiedPrivateOrderRequest) -> UnifiedPrivateOrderResult:
        if not request.dry_run:
            return UnifiedPrivateOrderResult(
                ...
                dry_run=False,
                message="Bitunix live trading is disabled in Sprint 26.",
            )
        return UnifiedPrivateOrderResult(
            accepted=True,
            exchange=self.exchange,
            ...
            quantity=request.quantity,
            dry_run=True,
            exchange_order_id=f"bitunix_dryrun_{uuid.uuid4().hex}",
            message="Bitunix order accepted in dry-run mode. No live order was sent.",
        )
